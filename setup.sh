#!/usr/bin/env bash
# This script sets up the GitHub Pages repository for deployment using Hugo. 
# This script needs to be run ONLY ONCE
# When setup has been done, the `deploy.sh` script has to be run to deploy changes and update the website. 
# Credits and Reference: https://hjdskes.github.io/blog/deploying-hugo-on-personal-github-pages/index.html
...
touch README.md
msg "Deleting the original \`master\` branch"
git branch -D master
